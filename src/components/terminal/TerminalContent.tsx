...
interface TerminalContentProps {
  ...
  onFileRun: (command: string) => Promise<void>;
}
...
export const TerminalContent: React.FC<TerminalContentProps> = ({
  ...
  // Handle file execution in the terminal
  const handleFileRun = (output: string, command: string) => {
    // Run the command in our terminal execution engine
    onFileRun(command);
  };
  ...
  return (
    <div className="flex-grow flex flex-col space-y-2 m-0">
      ...
      <div className="flex-grow overflow-auto bg-slate-900/50 rounded-md border border-slate-800">
        <CommandOutput />
      </div>
